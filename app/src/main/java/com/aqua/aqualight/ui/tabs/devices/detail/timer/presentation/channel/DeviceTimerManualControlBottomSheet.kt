...
class DeviceTimerManualControlBottomSheet : BottomSheetDialogFragment(
    ...
    companion object {
        ...
        fun show(
            ...
            val tag = TAG_PREFIX + request.requestKey
            if (fragmentManager.findFragmentByTag(tag) != null || fragmentManager.isStateSaved) {
                return
            }
            DeviceTimerManualControlBottomSheet().apply {
                arguments = bundleOf(
                    ARG_TITLE to request.header.title,
                    ARG_MESSAGE to request.header.message,
                    ARG_DURATION_LABEL to request.header.durationLabel,
                    ARG_ON_TEXT to request.options.onText,
                    ARG_OFF_TEXT to request.options.offText,
                    ARG_15_TEXT to request.options.duration15Text,
                    ARG_30_TEXT to request.options.duration30Text,
                    ARG_60_TEXT to request.options.duration60Text,
                    ARG_CUSTOM_TEXT to request.options.customText,
                    ARG_START_TEXT to request.actions.startText,
                    ARG_RESUME_TEXT to request.actions.resumeText,
                    ARG_RESUME_VISIBLE to request.resumeVisible,
                    ARG_INITIAL_REGIME to request.initialRegime.name,
                    ARG_REQUEST_KEY to request.requestKey
                )
...
internal data class DeviceTimerManualControlRequest(
    val header: DeviceTimerManualControlHeader,
    val options: DeviceTimerManualControlOptions,
    val actions: DeviceTimerManualControlActions,
    val resumeVisible: Boolean,
    val initialRegime: DeviceTimerChannelRegime,
    val requestKey: String
)

internal data class DeviceTimerManualControlHeader(
    val title: String,
    val message: String,
    val durationLabel: String
)

internal data class DeviceTimerManualControlOptions(
    val onText: String,
    val offText: String,
    val duration15Text: String,
    val duration30Text: String,
    val duration60Text: String,
    val customText: String
)

internal data class DeviceTimerManualControlActions(
