...
internal object AquariumTankTaxonomyTextResolver {
    ...
    private val tankTypes = listOf(
        ...
        Choice(
            ...
            R.string.aquarium_tank_profile_marine_fish
        ),
        Choice(
    ...
    fun canonicalWaterEnvironment(
        value: String,
        labelFor: (Int) -> String
    ): String? = canonical(value, waterEnvironments, labelFor)

    fun waterEnvironmentLabel(
        value: String,
        labelFor: (Int) -> String
    ): String = label(value, waterEnvironments, labelFor)

    fun canonicalTankType(
        value: String,
        labelFor: (Int) -> String
    ): String? = canonical(value, tankTypes, labelFor)

    fun tankTypeLabel(
        value: String,
        labelFor: (Int) -> String
    ): String = label(value, tankTypes, labelFor)

    fun canonicalTankStyle(
        value: String,
        labelFor: (Int) -> String
    ): String {
    ...
    fun tankStyleLabel(
        value: String,
        labelFor: (Int) -> String
    ): String = label(value, presetStyles, labelFor)
    ...
    private fun label(
        ...
        return choices
            ...
            ?.let { choice -> labelFor(choice.labelRes) }
            ?: trimmed
    }
}
