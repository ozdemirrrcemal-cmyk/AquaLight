...
class AquariumTankAdapter(
  ...
  class TankViewHolder(
    ...
    fun bind(
      ...
      binding.tvTankDay.text = getTankDayText(context, tank.setupDateMillis)
      binding.tvCareInfo.text = getCareInfoText(context, careSummary)
      binding.tvTankSize.text = getTankSizeText(context, tank)
      binding.tvSetupDate.text = getSetupDateText(context, tank.setupDateMillis)
    ...
    private fun getCareInfoText(context: Context, careSummary: TankCareSummaryUi?): String {
    ...
    private fun getSetupDateText(context: Context, setupDateMillis: Long?): String {
      ...
      return context.getString(
        ...
        AppLocaleFormatter.formatDate(context, setupDateMillis)
      )
    }

    private fun getTankDayText(context: Context, setupDateMillis: Long?): String {
      val setupMillis = setupDateMillis ?: System.currentTimeMillis()
      val day = TimeUnit.MILLISECONDS
        .toDays(System.currentTimeMillis() - setupMillis)
        .coerceAtLeast(0)
