...
class TankNameFragment : Fragment(R.layout.fragment_tank_name), TankStepFragment {
    ...
    private fun setupExistingValue() {
        val currentName = viewModel.tankDraft.name

        if (currentName.isNotBlank()) {
            binding.etTankName.setText(currentName)
            binding.etTankName.setSelection(currentName.length)
        }
    }

    private fun setupInputListener() {
        binding.etTankName.doAfterTextChanged {
            binding.tilTankName.error = null
        }
    }

    override fun validateAndSave(): Boolean {
        val tankName = binding.etTankName.text
            ?.toString()
            ?.trim()
            .orEmpty()

        if (tankName.isBlank()) {
            binding.tilTankName.error = "Tank name is required"
            binding.etTankName.requestFocus()
            return false
        }

        if (tankName.length < 2) {
            binding.tilTankName.error = "Tank name must be at least 2 characters"
            binding.etTankName.requestFocus()
            return false
        }

        binding.tilTankName.error = null
        viewModel.updateTankName(tankName)
