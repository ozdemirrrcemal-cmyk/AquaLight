...
internal object DeviceLightV1JsonParser {
    ...
    internal object Policy {
        fun parsePolicy(
            ...
            auto.requireLightKeys(
                if (managedAutoPlanSupported) {
                    AUTO_POLICY_KEYS_MANAGED_PLAN
                } else {
                    AUTO_POLICY_KEYS_BASE
                },
                "light.status.policy.auto"
            )
            val autoPolicy = if (managedAutoPlanSupported) {
                DeviceLightAutoPolicy(
            ...
            validateAutoPolicy(autoPolicy, managedAutoPlanSupported)
        ...
        private fun validateAutoPolicy(
            policy: DeviceLightAutoPolicy,
            managedAutoPlanSupported: Boolean
        ) {
            require(policy.capacity == DeviceLightRuntimeContract.Limit.AUTO_PROGRAM_CAPACITY)
            require(policy.timeStepMs == DeviceLightRuntimeContract.Limit.SCHEDULE_TIME_STEP_MS)
            require(policy.rampDurationsMs == EXPECTED_RAMPS)
            if (managedAutoPlanSupported) {
                require(
                    policy.managedPlanPhaseCapacity ==
                        DeviceLightRuntimeContract.Limit.MANAGED_PLAN_PHASE_CAPACITY
                )
                require(
                    policy.managedPlanTransitionDaysMax ==
                        DeviceLightRuntimeContract.Limit.MANAGED_PLAN_TRANSITION_DAYS_MAX
                )
                require(policy.managedPlanSameDayOnly)
                require(policy.managedPlanContiguous)
            }
        }
    ...
    internal object Activity {
        fun parseAutoSummary(
            ...
            val result = DeviceLightAutoSummary(
                ...
                planInstalled = managedAutoPlanSupported &&
                    data.requireLightBoolean("planInstalled"),
