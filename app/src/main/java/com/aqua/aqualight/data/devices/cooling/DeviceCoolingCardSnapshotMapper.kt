...
internal object DeviceCoolingCardSnapshotMapper {
    fun map(state: DeviceCoolingRuntimeState): DeviceCoolingCardSummary? {
        val control = DeviceCoolingControlSnapshotMapper.map(state) ?: return null
        val config = state.config ?: return null
        val programStatus = state.status?.program ?: return null
        val programRuntime = control.programRuntime ?: return null
        ...
        return DeviceCoolingCardSummary(
            mode = control.mode,
