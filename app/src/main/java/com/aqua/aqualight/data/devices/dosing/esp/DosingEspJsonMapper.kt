...
object DosingEspJsonMapper {
    ...
    fun fixedGpioPwmForChannelIndex(
        channelIndex: Int
    ): String {
        return DosingGpioPwmMapping.fixedGpioPwmForChannel(
    ...
    fun parseDosingRuntimeChannels(
        ...
        return List(
            ...
            parseChannelState(
                ...
                gpioPwm = fixedGpioPwmForChannelIndex(
                    channelIndex = channelIndex
                )
            )
        }
    ...
    fun parseDosingState(
        ...
        val parsedChannelState =
            parseChannelState(
                json = channelJson
            )

        val channelState =
            parsedChannelState.copy(
    ...
    fun createMergedCustomPeriodsSchedulePayload(
        ...
        val newTimers =
            safePeriods.mapIndexed { periodIndex, period ->
                DosingTimerSavePayload(
                    enabled = enabled,
    ...
    fun createMergedTimerModeSchedulePayload(
        ...
        val newTimers =
            safeDoses.mapIndexed { doseIndex, dose ->
                DosingTimerSavePayload(
                    enabled = enabled,
    ...
    private fun getAppTimerSlotIndicesForGpioPwm(
        gpioPwm: String
    ): List<Int> {
        val channelIndex =
            (0 until APP_TIMER_CHANNEL_COUNT).firstOrNull { index ->
                DosingGpioPwmMapping.areSameGpioPwm(
                    first = gpioPwm,
                    second = DosingGpioPwmMapping.fixedGpioPwmForChannel(
                        channelIndex = index
                    )
                )
            } ?: return emptyList()
    ...
    private fun createMergedTimerListPayload(
        ...
        val targetTimerSlotIndices =
            getAppTimerSlotIndicesForGpioPwm(
                gpioPwm = cleanTargetGpioPwm
            )

        require(
            value = targetTimerSlotIndices.isNotEmpty()
        ) {
            "Invalid GPIO PWM mapping: $cleanTargetGpioPwm"
        }

        val timerData =
            JSONObject()

        targetTimerSlotIndices.forEachIndexed { offset, timerIndex ->
            val timer =
                newTimers.getOrNull(
                    index = offset
                )
    ...
    private fun selectPrimaryTimer(
        ...
        channelTimers.firstOrNull { timer ->
        ...
        channelTimers.firstOrNull()?.let { timer ->
            return timer
        }
