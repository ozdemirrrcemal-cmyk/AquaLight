...
class UserPreferencesManager private constructor(
     ...
     *
     * - email ZORUNLU
     * - username / photoUrl / fullName OPSİYONEL
     * - null veya boş gelen değerler için ESKİ değer korunur
     *   (özellikle profilePhotoUrl'i asla yanlışlıkla "" yapmıyoruz)
     */
    suspend fun saveProfile(
        ...
        dataStore.updateData { prefs ->
            prefs.toBuilder()
                .setEmail(email) // email her zaman güncelleniyor
                .setUsername(
                    username ?: prefs.username
                )
                .setProfilePhotoUrl(
                    // Yeni url dolu ise onu kullan, değilse mevcut fotoğrafı koru
                    photoUrl?.takeIf { it.isNotBlank() } ?: prefs.profilePhotoUrl
                )
                .setFullName(
    ...
    // 🔹 Sadece profil foto'yu güncelle (EditProfile ekranı için)
