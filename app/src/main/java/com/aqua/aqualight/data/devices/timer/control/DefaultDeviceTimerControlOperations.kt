...
import com.aqua.aqualight.data.devices.runtime.modules.timer.clearChannelDisplayName
import com.aqua.aqualight.data.devices.runtime.modules.timer.currentAuthoritativeState
import com.aqua.aqualight.data.devices.runtime.modules.timer.replaceSchedules
import com.aqua.aqualight.data.devices.runtime.modules.timer.setChannelDisplayName
import com.aqua.aqualight.data.devices.runtime.modules.timer.setChannelRegime
import com.aqua.aqualight.data.devices.runtime.modules.timer.setTemporaryOverride
...
internal class DefaultDeviceTimerControlOperations(
    ...
    private val gateway = DeviceTimerControlGateway(devicesRepository)

    override fun observeControl(deviceUid: String): Flow<DeviceTimerControlResult> =
        gateway.observeControl(deviceUid)

    override fun currentControl(deviceUid: String): DeviceTimerControlResult =
        gateway.currentControl(deviceUid)

    override suspend fun refreshControl(deviceUid: String): DeviceTimerControlResult =
        gateway.refreshStatus(deviceUid = deviceUid, slotId = null)

    override suspend fun refreshChannel(
        deviceUid: String,
        slotId: String
    ): DeviceTimerControlResult = gateway.refreshStatus(deviceUid = deviceUid, slotId = slotId)

    override suspend fun setRegime(
        deviceUid: String,
        slotId: String,
        regime: DeviceTimerChannelRegime
    ): DeviceTimerControlResult = gateway.executeChannel(deviceUid, slotId) { resolved, channelKey ->
...
private class DeviceTimerControlGateway(
    private val devicesRepository: DevicesRepository
) {
