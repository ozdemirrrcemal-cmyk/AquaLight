...
/**
 * Authoritative Android mirror of the AquaLight WebSocket v2 wire contract.
 *
 * Protocol v2 deliberately has no v1 compatibility path. Runtime credentials are
 * used only as input to the HMAC handshake and are never serialized to the wire.
 */
object AqlWsContract {
    const val SCHEMA = "aql.ws.v2"
    const val SCHEMA_VERSION = 2
    const val DEFAULT_PATH = "/aql/v2/ws"
    ...
    // WebSocket v2 has no unauthenticated application command surface. Public
    // onboarding metadata remains on the bounded UDP/BLE provisioning contracts.
