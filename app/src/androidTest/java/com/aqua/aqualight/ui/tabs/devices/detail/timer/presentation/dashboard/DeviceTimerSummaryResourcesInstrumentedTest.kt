...
class DeviceTimerSummaryResourcesInstrumentedTest {
    ...
    fun activeChannelSummaryFormatsEveryPlaceholderInSupportedLocales() {
        ...
        assertEquals(
            "Açık çıkış: 1 / 4",
            turkish.activeSummary(active = 1, total = TOTAL_OUTPUT_COUNT)
        )
        assertEquals(
            "Açık çıkış: 3 / 4",
            turkish.activeSummary(active = MULTIPLE_ACTIVE_OUTPUT_COUNT, total = TOTAL_OUTPUT_COUNT)
        )
        assertEquals(
            "1 of 4 output active",
            english.activeSummary(active = 1, total = TOTAL_OUTPUT_COUNT)
        )
        assertEquals(
            "3 of 4 outputs active",
            english.activeSummary(active = MULTIPLE_ACTIVE_OUTPUT_COUNT, total = TOTAL_OUTPUT_COUNT)
        )
    }
...
private const val MULTIPLE_ACTIVE_OUTPUT_COUNT = 3
private const val TOTAL_OUTPUT_COUNT = 4
