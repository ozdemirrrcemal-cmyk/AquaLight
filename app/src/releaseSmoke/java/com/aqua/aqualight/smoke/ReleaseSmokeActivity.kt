...
class ReleaseSmokeActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        AppCompatDelegate.setApplicationLocales(
            LocaleListCompat.forLanguageTags(SMOKE_LANGUAGE_TAG)
        )
    ...
    private fun verifyVisibleIconControlDescriptions(root: View, screenName: String) {
        ...
        root.forEachDescendantInclusive { view ->
            ...
            val isIconOnlyControl = when (view) {
                is ImageButton -> true
                is ImageView -> view.isClickable
                is MaterialButton -> view.isClickable && view.icon != null && view.text.isNullOrBlank()
                else -> false
            }
        ...
        check(missingDescriptions.isEmpty()) {
            "$screenName has visible icon-only controls without descriptions: " +
