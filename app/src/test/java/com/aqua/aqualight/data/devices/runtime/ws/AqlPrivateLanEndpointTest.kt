...
class AqlPrivateLanEndpointTest {
    ...
    fun `route hides validated private ip behind app-owned hostname`() {
        ...
        assertTrue(route.url.endsWith(".device.aql.local:80/aql/v1/ws"))
    ...
    @Test
    fun `public addresses and incompatible protocol endpoints are rejected`() {
        ...
        assertFalse(
            DeviceRuntimeEndpoint(
                ip = "192.168.1.42",
                wsPort = 80,
                wsProtocolVersion = AqlWsContract.PROTOCOL_VERSION + 1
            ).hasWebSocketEndpoint
        )
        assertTrue(
            AqlPrivateLanEndpoint.route(
