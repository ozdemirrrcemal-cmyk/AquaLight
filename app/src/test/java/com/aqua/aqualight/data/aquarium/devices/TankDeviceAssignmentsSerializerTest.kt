...
class TankDeviceAssignmentsSerializerTest {
    ...
    @Test
    fun `serializer round trips valid assignments`() = runBlocking {
    ...
    fun `serializer rejects duplicate device assignment for one owner`() = runBlocking {
    ...
    @Test(expected = CorruptionException::class)
    fun `serializer rejects malformed protobuf bytes`() = runBlocking {
        TankDeviceAssignmentsSerializer.readFrom(
