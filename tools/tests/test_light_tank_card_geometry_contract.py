...
class LightTankCardGeometryContractTest(unittest.TestCase):
    def test_schedule_metrics_are_symmetric_around_divider(self):
        source = SPOTLIGHT.read_text(encoding="utf-8")
        schedule = source[
            source.index("private fun LightScheduleRow"):
            source.index("private fun LightScheduleMetric")
        ]

        self.assertIn("horizontalAlignment = Alignment.End", schedule)
        self.assertIn("horizontalAlignment = Alignment.Start", schedule)
        self.assertEqual(
            2,
            schedule.count(
