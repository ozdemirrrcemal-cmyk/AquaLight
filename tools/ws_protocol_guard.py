#!/usr/bin/env python3
"""Fail CI when the Android WebSocket v2 security contract drifts."""
...
FIXTURE_PATH = ROOT / "protocol/fixtures/aql_ws_v2_golden.json"
FIXTURE_SHA256 = "ccab0a132208030575b718e611aff4269d155b495c45422ccbb0c4a28fc150ef"
errors: list[str] = []
...
for token, reason in (
    ('const val SCHEMA = "aql.ws.v2"', "schema must remain v2"),
    ('const val DEFAULT_PATH = "/aql/v2/ws"', "legacy socket paths are forbidden"),
    ('const val PROTOCOL_VERSION = 2', "protocol version must remain explicit"),
...
for token in ('"aql.ws.v1"', '"/aql/v1/ws"', 'const val TOKEN = "token"'):
    forbid(contract_path, contract, token, "v1/plain-token compatibility must not return")
...
try:
    ...
    if access["public"]:
        errors.append("WebSocket v2 must not expose unauthenticated application commands")
