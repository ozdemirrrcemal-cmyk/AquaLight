# Stage 3 — Final Dependency Matrix
...
A process-scoped object must not retain `DevicesRepository`,
`TankDeviceAssignmentRepository`, runtime tokens, WebSocket clients or a previously
...
The `currentRepository(ownerUid)` methods are read-only dependency handoff points.
...
`DevicesRepositoryProvider.get` and
`TankDeviceAssignmentRepositoryProvider.get` remain temporarily contained inside
the foreground owner-session data boundary because they implement the already
validated awaited shutdown barrier from Stages 1–2. They are forbidden in UI,
ViewModels, `AppContainer`, and all ViewModel factories.

The composition root consumes only `currentRepository(ownerUid)`. This preserves
one runtime instance and avoids a risky Stage 1 lifecycle rewrite during Stage 3.
A later replacement of the two lifecycle holders must keep the same open/clear
barrier and can be performed without changing UI or ViewModel construction.
...
- Debug/Release unit tests, lint, minified Release, CodeQL and API 27/API 35 emulator
  workflows remain mandatory on the branch head.

## Physical merge gate

Code-level commercial approval does not replace physical provisioning regression.
QR, Nearby Scan, cancellation, rollback, Wi-Fi failure, process recreation and owner
switch scenarios must pass on the final Release Candidate before merge.
