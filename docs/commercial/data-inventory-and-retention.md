...
| Feedback destruction audit | Pseudonymous administrator Firebase UID (`actorUid`), record creation time, deletion cutoff, deleted-record count, operation mode and schema version | Evidence that scheduled personal-data destruction was performed; security and accountability | Cloud Firestore `retention_audits` in `europe-west1`, Belgium; readable only by an allow-listed feedback administrator | Legal obligation and accountability; subject to final legal review | Three years from creation. Unless another legal retention obligation applies, delete or anonymise in the first monthly destruction cycle after the three-year period ends. |
...
- Deletion requires a second explicit confirmation, is limited to 100 documents per batch, validates every returned document path and writes a minimal `retention_audits` record containing the pseudonymous administrator UID in the same atomic batch.
- The operator repeats bounded batches until dry-run reports zero and records the date and outcome. Destruction evidence is retained for three years from creation and, unless another legal retention obligation applies, deleted or anonymised in the first monthly destruction cycle after that period ends. Missing a monthly review is an incident requiring prompt cleanup and documentation.
...
- KVKK deletion, destruction and anonymisation regulation (Article 7/3 audit-record retention): https://www.kvkk.gov.tr/Icerik/5441/KISISEL-VERILERIN-SILINMESI-YOK-EDILMESI-VEYA-ANONIM-HALE-GETIRILMESI-HAKKINDA-YONETMELIK
