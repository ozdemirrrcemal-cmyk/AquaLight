...
| W0.1 / K03.13 | Kullanıcı deneyimi + kimyasal semantik standardı olarak kararlaştırıldı — 26.09.2026 | `GENERAL_HARDNESS` aquarium GH/total-hardness metric'i; canonical unit `mg/L as CaCO3`. UI `Genel sertlik (GH)` kalır ve verified aquarium test dGH veriyorsa kullanıcı dGH değerini aynen girer. Same-semantic conversion: `1 mg/L as CaCO3 = 0.056 °dH` (yaklaşık `1 dGH = 17.9 mg/L as CaCO3`). GH, KH/alkalinity veya elemental Ca/Mg değildir; GH'den Ca/Mg ayrı ayrı tahmin edilmez, elemental Ca+Mg doğrudan toplanıp GH yapılmaz. Unknown hardness/ppm basis fail-closed. | Ana sözleşme §6.15, §6.1, §25.1, §43.4 ve §46 |
| W0.1 / K03.14 | Kullanıcının profesyonel çözüm talebiyle kararlaştırıldı — 26.09.2026 | Tatlı suda aynı örnek/olaya ait doğrulanmış TAN (`mg/L as N`), pH ve sıcaklıktan EPA/Emerson denge denklemiyle serbest NH3 tahmini hesaplanır; NH3-N → NH3 kütle temeli açık dönüştürülür. V1 ürün sınırı pH 7.0–10.2, 6–32 °C; sınır dışında, farklı örneklerde veya eksik/güvensiz girdilerde sayı üretilmez. Deniz/resif için v1 otomatik hesap yok; doğrulanmış doğrudan serbest NH3 testi kabul edilir. Doğrudan ölçüm birincildir; hesaplanan değer ayrı, kaynakları ve sürümüyle saklanır, TAN ile iki kez kanıt sayılmaz; toksisite eşiği bu kararın parçası değildir. | Ana sözleşme §6.16, §6.3–6.5, §43.4 ve W4.10 |
| W0.1 / K04 | Kullanıcıyla kararlaştırıldı — 26.09.2026 | Tek birleşik status yerine bağımsız `hazardSeverity` (`NONE/ADVISORY/WARNING/CRITICAL`, değerlendirilemeyende null), kural başına yön (`BELOW/WITHIN/ABOVE`), kapsamlı `coverage` (`NONE/PARTIAL/COMPLETE`) ve yapılandırılmış `conflicts` + `conflictCoverage` kullanılacak. En yüksek kanıtlı tehlike başlığa çıkar; eksik veri ve çatışma ayrıca görünür kalır. Sıfır kural değerlendirildiyse normal denmez; yalnız tam değerlendirilen adlandırılmış kapsama özgü normal gösterilebilir. Canlı/bitki uyumluluğu iddiası için çatışma kapsamı da tamamlanmalı. SOFT kaynak kendiliğinden CRITICAL sayılmaz; sayısal eşikler onaylanmadı. | Ana sözleşme §14–15, §33–34, §43.1 ve §46 |

Bu kayıt kabul edilen kararları belgeler; paketlerin/kodun uygulanmış olduğunu göstermez. W0.1, K03 uygulama/veri işleri ve K05–K18 açık olduğu için kapanmaz. **K03.0–K03.14 ölçüm semantiği ve K04 çok boyutlu durum modeli kabul edildi.** K03 altında doğrulanmış test/cihaz profil kataloğu, conversion revision/hassasiyeti, method range/detection-limit metadata'sı ve golden-vector test verilerinin uygulanması gerekir. Ek ölçümleri açma ve yardım etkileşimi K13/UI kararlarında; input precision/bounded-result politikası K11'de tamamlanacaktır. **Sıradaki bağımsız karar K05: canlı aralık sınırı ve approximate/SOFT semantiği.**
...
| K04 | Karar alındı; uygulama açık | §14–15 artık tehlike, yön, kapsam ve çatışmayı ayrı taşır; `CRITICAL + PARTIAL + conflict` korunur. | §14 birleştirme/başlık önceliğini, §43.1 kombinasyon testlerini uygula. Eşik/rule evidence W0.4'te; SOFT güven düzeyi K05/K18'de ayrıca kesinleşecek. |
...
- [ ] W1.1 K02 kararına göre mevcut AquariumWaterParameter ve AquariumWaterSnapshot modellerini kontrollü genişlet; K03.1–K03.13 anlam/birimleriyle `ELECTRICAL_CONDUCTIVITY`, `TDS_REPORTED_PPM`, direct/calculated CO2, `POTASSIUM_CONCENTRATION`, `IRON_CONCENTRATION` + typed iron scope ve `GENERAL_HARDNESS` canonical mg/L-as-CaCO3 desteğini ekle. Direct/derived/method-scope provenance modelde ayrı olsun.
...
- [ ] W9.3 K03.2–K03.13 semantic/unit/source tests geçsin: önceki chemistry testlerine ek olarak GH dGH↔mg/L-as-CaCO3 golden vectors; raw source round-trip; GH/KH/alkalinity semantic separation; GH→elemental Ca/Mg inference rejection; elemental Ca+Mg naive-GH rejection; unknown hardness/ppm basis rejection; unsupported semantic→INSUFFICIENT_DATA.
