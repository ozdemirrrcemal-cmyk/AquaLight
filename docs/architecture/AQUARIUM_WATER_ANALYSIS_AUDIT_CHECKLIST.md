...
| İletkenlik/TDS, CO2, demir ve potasyum ek ölçümleri | §25.2–25.3; W0.2, W0.6, W8.1 | **Conductivity/TDS K03.10 ile kesinleşti:** TDS kullanıcı için ppm as-reported; factor UI'da sorulmaz, yalnız backend conversion gerektiğinde profile metadata'dır. CO2, Fe/K semantiği açık |
...
| K03 | Kısmen kararlaştırıldı; kalanlar kritik | K03.0–K03.9'a ek olarak K03.10 ile conductivity=`µS/cm`, TDS=`ppm as reported`, user-facing factor yok, verified-profile EC↔TDS conversion only, temperature/ATC provenance ve no-double-counting kabul edildi. | Sıradaki K03.11: CO2 direct-vs-calculated semantics. Ardından Fe/K ve diğer ek ölçümler; kanıtlı profile/precision tamamlanmalı. |
...
- [ ] W0.6 K03.5–K03.10 politikalarını uygula: salinity/SG, alkalinity, DO, chlorine, reef Ca/Mg ve conductivity/TDS typed semantics. Conductivity canonical µS/cm + temperature/ATC basis; TDS source-native ppm olarak saklanır, UI factor istemez; EC↔TDS yalnız verified factor/scale ile çevrilir ve same-meter outputs double-count edilmez. Ardından §25.2'deki CO2/Fe/K ve diğer ek ölçüm semantiklerini kesinleştir.
...
- [ ] W1.1 K02 kararına göre mevcut AquariumWaterParameter ve AquariumWaterSnapshot modellerini kontrollü genişlet; K03.1–K03.10 anlam/birimleriyle mevcut metric'lere `ELECTRICAL_CONDUCTIVITY` ve `TDS_REPORTED_PPM` ekle; ppm/dKH/KH/%/hardness etiketlerini anlam değiştirerek sessizce kullanma. TDS factor normal kullanıcı girdisi değil source-profile metadata olsun.
...
- [ ] W9.3 K03.2–K03.10 semantic/unit/source tests geçsin: önceki kimya testlerine ek olarak conductivity µS/cm/mS scale normalization; raw vs ATC/reference basis; TDS reported ppm round-trip; factor unknownken TDS save; factor bilinmeden EC↔TDS conversion rejection; 0.5/0.7 profile conversion golden vectors; same-meter EC+TDS no-double-count; unknown-scale catalog TDS hard-assessment rejection; unsupported→INSUFFICIENT_DATA; locale/precision sınırları.
