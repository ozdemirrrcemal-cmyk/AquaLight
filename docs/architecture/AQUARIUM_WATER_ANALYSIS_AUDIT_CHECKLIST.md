...
Bu kayıt kabul edilen kararları belgeler; paketlerin/kodun uygulanmış olduğunu göstermez. W0.1, K03'ün kalan kararları ve K04–K18 açık olduğu için kapanmaz. **K03.0 ölçüm kapsamını, K03.1 NO3/NO2/PO4 kanonik kayıt anlamı ve birimlerini, K03.2 test/cihaz seçimi + kaynak semantiği çözümleme + normalizasyon akışını, K03.3 amonyak kanonik temellerini, K03.4 çok-sonuçlu aynı-olay cardinality/UI davranışını, K03.5 deniz salinity/SG dönüşüm güvenliğini, K03.6 alkalinite/KH semantiğini ve K03.7 çözünmüş oksijen mg/L/% saturation politikasını kapatır.** Sıradaki karar: **W0.1 / K03.8 — serbest klor / toplam klor / kloramin semantiği, kaynak-su vs tank-su bağlamı ve aynı testte çoklu sonuç davranışı**. Tam ticari test kataloğu içerikleri, kanıtlı dönüşüm katsayıları/hassasiyetleri ve diğer ek ölçümlerin semantiği kendi maddelerinde tamamlanacaktır. Ek ölçümleri açma ve yardım etkileşimi ayrıca kararlaştırılacak; kapsam onayı yeni görsel düzen onayı değildir.

K03.1'in doğrulanan kaynakları ile kabul edilen K03.2 kaynak-seçim/normalizasyon, K03.3 amonyak kanonik, K03.4 çok-sonuçlu kayıt/UI, K03.5 deniz salinity/SG, K03.6 alkalinite/KH ve K03.7 çözünmüş oksijen güvenlik davranışı: [Konsantrasyon birimleri araştırma notu](research/WATER_ANALYSIS_CONCENTRATION_UNITS_K03.md).
...
| Deniz/resif için tuzluluk, alkalinite, Ca/Mg, NO3/PO4; GH'nin bunların yerine geçmemesi | §25.1; W0.5–W0.6, W1.1, W8.1 | Tuzluluk K03.5 ile; alkalinite K03.6 ile kesinleşti. Marine alkalinity=`TOTAL_ALKALINITY`/meq/L; ikinci KH alanı yok. Ca/Mg ve kalan ek parametre semantikleri açık |
| Çözünmüş oksijen, serbest/toplam klor desteği ve ölçülmeyen değeri türetmeme | §25.2; W0.2, W0.6, W4.6 | **Çözünmüş oksijen K03.7 ile kesinleşti:** mg/L O2 kanonik; % saturation ayrı ve context-dependent. Serbest/toplam klor/kloramin semantiği K03.8 kararını bekliyor |
...
| K03 | Kısmen kararlaştırıldı; kalanlar kritik | K03.0–K03.6'ya ek olarak K03.7 ile `DISSOLVED_OXYGEN_CONCENTRATION`=`mg/L O2`, `% saturation` ayrı metric/representation, same-observation no-double-count, same-event temperature/salinity-or-SC/pressure prerequisites ve historical-context güvenliği kabul edildi. | Sıradaki K03.8: free chlorine / total chlorine / chloramine semantiği ve source-water vs tank-water context. Ardından Ca/Mg, conductivity/TDS, CO2, Fe/K ve diğer ek ölçümler; kanıtlı profile/precision tamamlanmalı. |
...
- [ ] W0.6 K03.5–K03.7 politikalarını uygula: salinity/SG/conductivity/mass-salinity ayrı typed metric; `TOTAL_ALKALINITY` canonical meq/L; verified alkalinity dönüşümleri; marine duplicate `KH`+`Alkalinite` yok; freshwater alkalinity/carbonate-hardness/GH ayrımı korunur. DO için `DISSOLVED_OXYGEN_CONCENTRATION` canonical `mg/L O2`, `% saturation` ayrı metric/representation; same-event temperature + pressure reference + gerekiyorsa salinity/specific conductance olmadan cross-conversion yok; aynı probe observation double-count edilmez ve historical ölçüm bugünkü context ile normalize edilmez. Ardından §25.2'deki diğer ek ölçüm semantiklerini kesinleştir.
...
- [ ] W1.1 K02 kararına göre mevcut AquariumWaterParameter ve AquariumWaterSnapshot modellerini kontrollü genişlet; K03.1–K03.7 anlam/birimleriyle NO2, TAN, direct NH3, `PRACTICAL_SALINITY_PSS78`, `SPECIFIC_GRAVITY`, `TOTAL_ALKALINITY`, gerektiğinde `CARBONATE_HARDNESS`, `DISSOLVED_OXYGEN_CONCENTRATION`, `DISSOLVED_OXYGEN_SATURATION_PERCENT` ve §25 kapsamındaki eksik parametre desteğini ekle; ppm/dKH/KH/% gibi display etiketlerini anlam değiştirerek sessizce kullanma.
- [ ] W1.2 Input, raw measurements, record, parameter/entity assessment, reasons, recommendations, conflicts, missingData ve typed failure modellerini oluştur. Bir `WaterAnalysisRecord` aynı `observedAt` olayında TAN ve direct free NH3 gibi farklı canonical metric'leri birlikte taşıyabilsin; bunları tek ammonia slot/mode alanına sıkıştırma.
...
- [ ] W1.4 Parse/normalization öncesi anlamı koru; birim dönüşümünü UI metninden değil K03.2/K03.5/K03.7 typed source profile/measurement-basis-unit politikasından yap. Ham değer, source kind, profile/revision veya guided typed semantics, result mode, reference/sample temperature, pressure/compensation context ve normalize değer gerektiği yerde ayrı izlenebilir olsun. Salinity ve DO cross-representation dönüşümleri yalnız same-event prerequisites tam ve algoritma versioned ise çalışsın; device compensation iki kez uygulanmasın. Yuvarlama değerlendirme sonrasında yalnız sunumda olsun.
...
- [ ] W9.3 Birim/ammonia/marine/DO semantiği, K03.2–K03.7 source selection; concurrent multi-result; TAN/direct NH3; PSS-78/SG/conductivity/mass-salinity; alkalinity conversions; `DISSOLVED_OXYGEN_CONCENTRATION` mg/L O2 vs `% saturation`; same-event temperature/pressure/salinity-or-SC prerequisites; versioned DO conversion golden vectors; historical-context rejection; device compensation no-double-apply; same-probe mg/L+% no-double-count; unsupported conversion→INSUFFICIENT_DATA; unresolved-draft, source-change, provenance, locale, precision, nonfinite, negatif, aşırı büyük sayı, boş/sıfır ve zaman testleri geçsin.
