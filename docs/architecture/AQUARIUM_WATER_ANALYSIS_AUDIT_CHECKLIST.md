...
Bu kayıt kabul edilen kararları belgeler; paketlerin/kodun uygulanmış olduğunu göstermez. W0.1, K03'ün kalan kararları ve K04–K18 açık olduğu için kapanmaz. **K03.0 ölçüm kapsamını, K03.1 NO3/NO2/PO4 kanonik kayıt anlamı ve birimlerini, K03.2 test/cihaz seçimi + kaynak semantiği çözümleme + normalizasyon akışını, K03.3 amonyak kanonik temellerini, K03.4 çok-sonuçlu aynı-olay cardinality/UI davranışını, K03.5 deniz salinity/SG dönüşüm güvenliğini ve K03.6 alkalinite/KH semantiği ile duplicate-field önleme politikasını kapatır.** Sıradaki karar: **W0.1 / K03.7 — çözünmüş oksijen için kanonik metric/birim, sıcaklık-tuzluluk bağımlılığı ve doygunluk yüzdesi davranışı**. Tam ticari test kataloğu içerikleri, kanıtlı dönüşüm katsayıları/hassasiyetleri ve diğer ek ölçümlerin semantiği kendi maddelerinde tamamlanacaktır. Ek ölçümleri açma ve yardım etkileşimi ayrıca kararlaştırılacak; kapsam onayı yeni görsel düzen onayı değildir.

K03.1'in doğrulanan kaynakları ile kabul edilen K03.2 kaynak-seçim/normalizasyon, K03.3 amonyak kanonik, K03.4 çok-sonuçlu kayıt/UI, K03.5 deniz salinity/SG ve K03.6 alkalinite/KH güvenlik davranışı: [Konsantrasyon birimleri araştırma notu](research/WATER_ANALYSIS_CONCENTRATION_UNITS_K03.md).
...
| K03 | Kısmen kararlaştırıldı; kalanlar kritik | K03.0–K03.5'e ek olarak K03.6 ile marine `TOTAL_ALKALINITY`/meq/L, verified dKH–CaCO3 dönüşümleri, freshwater `TOTAL_ALKALINITY` vs `CARBONATE_HARDNESS` vs `GENERAL_HARDNESS` ayrımı ve duplicate KH/Alkalinite alan yasağı kabul edildi. | Sıradaki K03.7: çözünmüş oksijen semantiği ve birimleri. Ardından klor, Ca/Mg, conductivity/TDS, CO2, Fe/K ve diğer ek ölçümler; kanıtlı profile/precision tamamlanmalı. Semantik bilinmiyorsa motor tahmin etmez. |
...
- [ ] W0.6 K03.5–K03.6 deniz politikalarını uygula: salinity/SG/conductivity/mass-salinity ayrı typed metric; `TOTAL_ALKALINITY` canonical meq/L; verified same-semantic dKH/meq/L/mg/L as CaCO3 dönüşümleri; marine UI'da duplicate `KH` + `Alkalinite` alanı yok; freshwater `TOTAL_ALKALINITY`, `CARBONATE_HARDNESS`, `GENERAL_HARDNESS` ayrımı korunur; bare `KH`, `ppm`, `dKH` etiketi tek başına semantik otorite değildir. Ardından §25.2'deki diğer ek ölçüm birimlerini/yorum önkoşullarını kesinleştir. Mevcut UI tasarımı korunacak.
...
- [ ] W1.1 K02 kararına göre mevcut AquariumWaterParameter ve AquariumWaterSnapshot modellerini kontrollü genişlet; K03.1–K03.6 anlam/birimleriyle NO2, TAN, direct NH3, `PRACTICAL_SALINITY_PSS78`, `SPECIFIC_GRAVITY`, `TOTAL_ALKALINITY`, gerektiğinde `CARBONATE_HARDNESS` ve §25 kapsamındaki eksik parametre desteğini ekle; ppm/dKH/KH isimlerini anlam değiştirerek sessizce kullanma. NO2/amonyak UI alanları zaten mevcuttur; diğer alanların görünürlüğü §25'e bağlıdır.
...
- [ ] W1.4 Parse/normalization öncesi anlamı koru; birim dönüşümünü UI metninden değil K03.2/K03.5 typed source profile/measurement-basis-unit politikasından yap. Ham değer, source kind, profile/revision veya guided typed semantics, result mode, reference/sample temperature ve normalize değer gerektiği yerde ayrı izlenebilir olsun. Salinity/SG/conductivity/mass-salinity dönüşümleri yalnız prerequisites tam ve algoritma versioned ise çalışsın. Yuvarlama değerlendirme sonrasında yalnız sunumda olsun.
...
- [ ] W9.3 Birim/ammonia/marine semantiği, K03.2–K03.6 source selection; concurrent multi-result; TAN/direct NH3; PSS-78/SG/conductivity/mass-salinity; SG temperature/reference; ambiguous `ppt`; versioned conductivity→PSS-78; `TOTAL_ALKALINITY` meq/L; dKH↔mg/L as CaCO3↔meq/L golden vectors; bare `KH`/`ppm` semantic rejection; freshwater alkalinity/carbonate-hardness/GH separation; marine duplicate KH+alkalinity prevention; unsupported conversion→INSUFFICIENT_DATA; no-double-counting; unresolved-draft, source-change, provenance, locale, precision, nonfinite, negatif, aşırı büyük sayı, boş/sıfır ve zaman testleri geçsin.
