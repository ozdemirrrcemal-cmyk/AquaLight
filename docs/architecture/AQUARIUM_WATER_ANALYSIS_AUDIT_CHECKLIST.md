...
| W0.1 / K03.2 | Kullanıcıyla kararlaştırıldı — 26.09.2026 | Test/cihaz seçimi parametre bazlı tutulacak ve hatırlanacak. Doğrulanmış marka+model/yöntem profili ölçüm türünü, raporlama temelini, kaynak birimini ve varsa sonuç modunu belirleyecek; kullanıcı sonraki girişlerde yalnız sonucu girecek. Çok modlu üründe gerekli mod açıkça seçilecek, tahmin edilmeyecek. Ürün katalogda yoksa kontrollü ölçüm türü–raporlama temeli–birim listesiyle rehberli seçim yapılacak. Çözülemeyen alan normal analiz kaydına commit edilmeyecek; taslak/form durumunda kalabilecek ve diğer doğrulanmış ölçümler kaydedilebilecek. Ham değer + kaynak semantiği + profil/revision + normalize değer ayrı izlenecek; kaynak değişikliği girilmiş sayıyı sessizce yeniden anlamlandırmayacak. | Ana sözleşme §6.2, §6.5, §7, §28.1 ve §43.4 |
...
Bu kayıt kabul edilen kararları belgeler; paketlerin/kodun uygulanmış olduğunu göstermez. W0.1, K03'ün kalan kararları ve K04–K18 açık olduğu için kapanmaz. **K03.0 ölçüm kapsamını, K03.1 NO3/NO2/PO4 kanonik kayıt anlamı ve birimlerini, K03.2 test/cihaz seçimi + kaynak semantiği çözümleme + normalizasyon akışını, K03.3 ise amonyak kanonik temellerini kapatır.** Sıradaki karar: **W0.1 / K03.4 — aynı ölçüm olayında aynı çok-sonuçlu testten hem Toplam amonyak hem doğrudan Serbest amonyak sonucunun birlikte saklanıp saklanmayacağı ve UI davranışı**. Tam ticari test kataloğu içerikleri, kanıtlı dönüşüm katsayıları/hassasiyetleri ve diğer ek ölçümlerin semantiği kendi maddelerinde tamamlanacaktır. Ek ölçümleri açma ve yardım etkileşimi ayrıca kararlaştırılacak; kapsam onayı yeni görsel düzen onayı değildir.

K03.1'in doğrulanan kaynakları ile kabul edilen K03.2 kaynak-seçim/normalizasyon ve K03.3 amonyak kanonik davranışı: [Konsantrasyon birimleri araştırma notu](research/WATER_ANALYSIS_CONCENTRATION_UNITS_K03.md).
...
| Test/cihaz seçimiyle ölçüm türü, raporlama temeli ve birimi belirleme; seçimleri parametre bazlı hatırlama | §6.2, §6.5, §7, §28.1; W0.3, W1.4, W8.21 | **K03.2 kabul edildi:** doğrulanmış profil otomatik belirler; katalog dışı ürün rehberli typed seçimle girilebilir; kullanıcıya manuel dönüşüm yaptırılmaz |
| Doğrudan serbest NH3 sonucunu toplamdan ayrı kaydetme | §6.3–6.5; W0.3, W1.2 | **K03.3 ile kanonik temel kabul edildi:** doğrudan serbest NH3 `mg/L as NH3`; TAN veya hesaplanan NH3 yerine kullanılmaz. Aynı analiz olayında TAN + doğrudan NH3'ün birlikte saklanması K03.4 kararını bekliyor |
...
| K03 | Kısmen kararlaştırıldı; kalanlar kritik | K03.0 ölçüm kapsamı (§25), K03.1 NO3/NO2/PO4 kanonik anlam/birimleri (§6.1), K03.2 test/cihaz seçimi + kaynak çözümleme + normalizasyon akışı ve K03.3 TAN=`mg/L as N`, doğrudan NH3=`mg/L as NH3` kabul edildi. | Sıradaki K03.4: aynı ölçüm olayında çok-sonuçlu bir kitin TAN + doğrudan serbest NH3 sonuçlarını birlikte saklama/UI davranışı. Ardından tuzluluk/SG, KH/alkalinite ve ek ölçümler; kanıtlı profil/dönüşüm katsayıları ve precision tamamlanmalı. Çözülemeyen test semantiği commit edilmez; katalogdaki eski kaynak değerler yeni standarda göre kendiliğinden doğrulanmış sayılmaz. |
...
- [ ] W1.1 K02 kararına göre mevcut AquariumWaterParameter ve AquariumWaterSnapshot modellerini kontrollü genişlet; K03.1–K03.3 anlam/birimleriyle NO2, `TOTAL_AMMONIA_NITROGEN` (mg/L as N), `FREE_AMMONIA_NH3` (mg/L as NH3) ve §25 kapsamındaki eksik parametre desteğini ekle; ppm isimlerini anlam değiştirerek sessizce kullanma. NO2/amonyak UI alanları zaten mevcuttur; diğer alanların görünürlüğü §25'e bağlıdır.
...
- [ ] W1.4 Parse/normalization öncesi anlamı koru; birim dönüşümünü UI metninden değil K03.2 typed source profile/measurement-basis-unit politikasından yap. Ham değer, source kind, profile/revision veya guided typed semantics, result mode ve normalize değer ayrı izlenebilir olsun. Yuvarlama değerlendirme sonrasında yalnız sunumda olsun.
...
- [ ] W8.21 §6.5 K03.2–K03.3 akışını onaylı tasarımı bozmadan bağla: her parametre için küçük test/cihaz kontrolü; hatırlanan seçim; doğrulanmış profilden ölçüm türü/raporlama temeli/birim/sonuç modu; katalog dışı üründe rehberli typed fallback; kutuda kaynak birimi görünür ve dönüşüm application katmanında yapılır. Ana etiket `Toplam amonyak` kalır; doğrudan free result açıkça `Serbest amonyak (NH3)` olarak ayrılır. Kaynak değişimi mevcut sayıyı sessizce yeniden anlamlandırmasın. Semantiği çözülemeyen alan normal analize commit edilmesin, draft/form state'te kalabilsin; diğer doğrulanmış ölçümler kaydedilebilsin.
...
- [ ] W9.3 Birim/ammonia semantiği, K03.2 parametre bazlı test/cihaz seçimi, profil/guided fallback, çoklu sonuç modu, unresolved-draft davranışı, source-change sonrası sessiz yeniden anlamlandırmama, raw→normalized provenance round-trip, decimal locale, precision, nonfinite, negatif, aşırı büyük sayı, boş/sıfır ve zaman testleri geçsin.
