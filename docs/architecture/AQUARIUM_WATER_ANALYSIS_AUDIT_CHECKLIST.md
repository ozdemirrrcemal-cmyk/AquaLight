...
Bu kayıt kabul edilen kararları belgeler; paketlerin/kodun uygulanmış olduğunu göstermez. W0.1, K03'ün kalan kararları ve K04–K18 açık olduğu için kapanmaz. **K03.0 ölçüm kapsamını, K03.1 NO3/NO2/PO4 kanonik kayıt anlamı ve birimlerini, K03.2 test/cihaz seçimi + kaynak semantiği çözümleme + normalizasyon akışını, K03.3 amonyak kanonik temellerini, K03.4 çok-sonuçlu aynı-olay cardinality/UI davranışını ve K03.5 deniz salinity/SG dönüşüm güvenliğini kapatır.** Sıradaki karar: **W0.1 / K03.6 — deniz/resif alkalinite/KH semantiği, birimi ve aynı ölçümün iki farklı alan gibi gösterilmemesi**. Tam ticari test kataloğu içerikleri, kanıtlı dönüşüm katsayıları/hassasiyetleri ve diğer ek ölçümlerin semantiği kendi maddelerinde tamamlanacaktır. Ek ölçümleri açma ve yardım etkileşimi ayrıca kararlaştırılacak; kapsam onayı yeni görsel düzen onayı değildir.

K03.1'in doğrulanan kaynakları ile kabul edilen K03.2 kaynak-seçim/normalizasyon, K03.3 amonyak kanonik, K03.4 çok-sonuçlu kayıt/UI ve K03.5 deniz salinity/SG güvenlik davranışı: [Konsantrasyon birimleri araştırma notu](research/WATER_ANALYSIS_CONCENTRATION_UNITS_K03.md).
...
| K03 | Kısmen kararlaştırıldı; kalanlar kritik | K03.0–K03.4 kararlarına ek olarak K03.5 ile PSS-78/SG/conductivity/mass-salinity/vendor-ppt ayrımı, sıcaklık/reference provenance'ı, güvenli dönüşüm ve no-double-counting kabul edildi. | Sıradaki K03.6: alkalinite/KH semantiği ve birimleri. Ardından diğer ek ölçümler; kanıtlı profil/dönüşüm katsayıları ve precision tamamlanmalı. Çözülemeyen veya dönüştürülemeyen semantik tahmin edilmez; motor gerekirse `INSUFFICIENT_DATA` üretir. |
...
- [ ] W0.6 K03.5 deniz salinity politikasını uygula: `PRACTICAL_SALINITY_PSS78`, `SPECIFIC_GRAVITY`, conductivity ve açık mass/absolute-salinity basis'lerini ayrı typed metric tut; SG reference/calibration temperature ve gerekli sample temperature bilgisini koru; generic `ppt`'yi profile çözmeden normalize etme; SG↔salinity kör dönüşüm yapma; aynı source observation'dan türeyen gösterimleri bağımsız kanıt sayma. Ardından §25.1–25.2'deki alkalinite/KH ve diğer ek ölçüm birimlerini/yorum önkoşullarını kesinleştir. Mevcut UI tasarımı korunacak.
...
- [ ] W9.3 Birim/ammonia semantiği, K03.2–K03.5 source selection ve marine metric ayrımları; profile/guided fallback; concurrent multi-result round-trip; TAN/direct NH3 overwrite etmeme; PSS-78/SG/conductivity/mass-salinity ayrımı; SG temperature/reference prerequisites; ambiguous `ppt`; versioned conductivity→PSS-78 golden vectors; unsupported conversion→INSUFFICIENT_DATA; aynı sensor observation türevlerinin double-count edilmemesi; unresolved-draft, source-change, raw→normalized provenance, locale, precision, nonfinite, negatif, aşırı büyük sayı, boş/sıfır ve zaman testleri geçsin.
