...
Implementation clarification accepted on 26 September 2026: Water Analysis has **one canonical parameter identity and one normalized measurement path**. The existing `AquariumWaterParameter` / `AquariumWaterSnapshot` application boundary is migrated to those identities rather than accumulating parallel legacy and canonical fields. Catalog JSON keys such as `nitratePpm` are source-format details at the data ingestion boundary; they are not an independent domain metric. A catalog requirement or device result enters the shared comparison path only after its analyte, reporting basis and unit are verified and normalized. Unresolved source semantics remain raw provenance with a typed non-comparable reason, never a second numeric path or a guessed canonical value. Existing callers are migrated as part of the same change, and duplicate representations of one observation are not counted twice.

Implementation stage W1.1: the existing snapshot and parameter enum use canonical chemical identities, including separate measured TAN and direct free NH3. The bundled livestock JSON does not contain per-field source profiles or reporting-basis evidence. Its temperature and pH ranges remain comparable; the other parsed source strings are retained as `UNVERIFIED_SOURCE_SEMANTICS` (or `UNPARSEABLE_REQUIREMENT`) and cannot create a compatibility claim until evidence-backed ingestion profiles and normalization are added. This is an explicit coverage gap, not a replacement range or a second measurement model. Raw test/device provenance, analysis-record persistence and source-profile normalization are separate subsequent implementation stages governed by sections 5–7.
...
**Acropora catalog correction (26 September 2026):** Record `440cfc3ef12c07ff` previously stated PAR `200–400+`, which mixes an interval with an undefined upper-end `+`. Its PAR field is now `~200–300`: approximate genus-level placement guidance, not a strict lower/upper limit or a safety threshold. Bulk Reef Supply recommends 200–300 PAR across an Acropora-dominant tank, and Tidal Gardens reports around 300 PAR in its systems while also observing successful systems above 500 PAR. The value remains `SOFT` and is not eligible for exact-bound compatibility or a hard conflict. Sources reviewed 26 September 2026: [BRS Acropora lighting](https://www.bulkreefsupply.com/content/post/par-and-reef-tank-lighting-schedules-whats-the-ideal-program-for-led-aquarium-lighting-reef-faqs); [Tidal Gardens Acropora care](https://tidalgardens.com/articles/coral-care-articles/acropora-coral-care.html).
...
### 12.4 Accepted plant-catalog boundary (K06)

Accepted with the user on 26 September 2026. The existing `aqualight_plant_catalog.json` is the single packaged source for plant identity and care. Today `ui/tabs/aquarium/catalog/plant/AquariumPlantCatalog` opens, parses and caches that asset, while `PlantCatalog` adapts its 271 records into picker sections. Move that reader/parser/cache into `data/aquarium/catalog/plant/` and expose one read-only `PlantCareCatalogOperations` boundary with application-owned `AquariumPlantCatalogRecord` / care models. The picker and Aquarium Health context provider obtain the **same immutable catalog snapshot and revision** through that boundary; the pure engine receives already resolved care facts and performs no Android/JSON/catalog I/O. Do not create a second asset, parser, or UI-owned care cache.

- **Existing behavior:** preserve all stable `plant:*` identities, display/search names, placement, category membership/order, and picker navigation/result contract. Keep `PlantCatalog` as the UI-only section/localized-label projection and `AquariumPlant` as its presentation model. Its category rules and `AquariumPlantLightCatalog` light-demand lookup remain in place. The latter is a separate existing lighting policy keyed by the same identities, not a second source of water-care ranges. Validate its identity-set parity with the 271 plant records; do not silently drop a picker entry if the light lookup is missing.
...
- K06 single-source plant asset parity: 271 stable IDs, existing picker category/order/search behavior and lighting-catalog ID parity before/after the boundary move;
- one immutable plant snapshot/revision for picker and context, repeated/concurrent reads, content revision distinct from schema/count and lighting revision;
- malformed/unavailable plant asset and missing saved plant identity yield typed unavailable/missing results, not a valid empty catalog or hard assessment;
